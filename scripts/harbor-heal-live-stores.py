...
def curl(args, out):
    subprocess.check_call(["curl", "-sS", "-m", "45", "-c", COOKIE, "-b", COOKIE, *args, "-o", out])
    return Path(out).read_text(errors="replace")

def find_action(html, needle):
    for m in re.finditer(r'name="(\$ACTION_ID_[^"]+)"', html):
...
curl(
    ["-X", "POST", f"{BASE}/login/admin", "-F", f"{find_action(html, 'password')}=",
     "-F", "email=oscar.d@example.net", "-F", "password=HarborAdmin!2026"],
...
for store in load_stores():
    ...
    if not name or not slug:
        continue
    if slug in {"atlas-fitness", "brightbyte-electronics", "cedar-co-home", "lumen-beauty", "northline-outfitters", "willow-baby"}:
        continue
    wanted.append(store)

missing = [s for s in wanted if s["name"] not in merchants and s["slug"] not in merchants]
print("heal missing", [s["name"] for s in missing])
if not missing:
...
for store in missing:
    ...
    apps = curl([
        "-L", "-X", "POST", f"{BASE}/merchants/applications",
        "-F", f"{create_action}=",
        "-F", f"businessName={store['name']}",
        "-F", f"contactName={store.get('users', [{}])[0].get('name') or store['name']}",
        "-F", f"email={email}",
        "-F", f"phone={store.get('phone') or ''}",
        "-F", f"country={store.get('country') or 'Pakistan'}",
        "-F", "category=Hot Selling Items",
        "-F", "notes=Automatic persist heal",
...
for store in missing:
    ...
    for block in blocks:
        if email in block and 'name="id"' in block and "APPROVED" in block:
            mid = re.search(r'name="id" value="([^"]+)"', block)
            if mid:
                app_id = mid.group(1)
                break
    if not app_id:
    ...
    curl([
        ...
        "-F", f"{review_action}=",
        "-F", f"id={app_id}",
        "-F", "reviewNote=Automatic persist heal",
        "-F", "decision=APPROVED",
...
for mid in ids:
    ...
    store = next((s for s in missing if (s.get("email") or "").lower() in page), None)
    if not store:
        continue
    try:
    ...
    curl([
        "-L", "-X", "POST", f"{BASE}/merchants/{mid}",
        "-F", f"{user_action}=",
        "-F", f"merchantId={mid}",
        "-F", f"name={name}",
        "-F", f"email={email}",
        "-F", "password=HarborMerchant!2026",
    ], f"/tmp/heal-user-{mid}.html")

final = curl(["-L", f"{BASE}/merchants"], "/tmp/heal-mer-final.html")
print("FINAL", [(s["name"], s["name"] in final) for s in wanted])
