...
- **Build failed: GLIBC / SWC / next.config.ts:** Do **not** click **Fix and redeploy**. Hostinger’s Linux is older than Next.js native SWC. Latest **`main`** builds with `next build --webpack` so WASM SWC can run. Click **Close**, Deploy GitHub **`main`**, then **Restart**.
